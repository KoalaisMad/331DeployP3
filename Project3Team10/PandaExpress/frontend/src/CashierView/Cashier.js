...
export default function Cashier() {
  ...
  // Track whether there is an in-progress combo to gate other item flows
  const hasIncompleteCombo = combos.length > 0 && !combos[combos.length - 1].completed;
  
  // 2. DYNAMIC STATE FOR MENU CATEGORY LOOKUPS
  // Fetch sides and entrees lists based on is_side flag
  ...
  function getEntrees() {
    apiFetch('/entrees')
      .then(res => res.json())
      .then(data => setDynamicButtons(data.map(item => ({ id: item.id, label: item.name })))) // Fixed syntax here
      .catch(console.error);
  }
  ...
  function onAddDrink() {
      if (hasIncompleteCombo) {
          console.warn('Cannot add a drink while there is an incomplete combo.');
          return;
      }

      // Open a modal or dynamic button grid for drink size selection
      setDynamicButtons([
          { id: 1, label: 'Small Drink', price: 2.00 },
          { id: 2, label: 'Medium Drink', price: 2.00 },
          { id: 3, label: 'Large Drink', price: 2.00 }
      ]);

      setCurrentCategory('drinks');
      
      // FIX: Enable the bottom menu so the buttons are clickable
      setBottomMenuEnabled(true); 
  }
  function onAppetizersSelected() {
    if (hasIncompleteCombo) {
      alert('Finish the current combo before adding a different item.');
      return;
    }
    getAppetizers();
    setBottomMenuEnabled(true);
  ...
  function onDynamicButtonClick(btn) {
    if (currentCategory === 'drinks') {
        // Add the selected drink to the order with its price
        setDrinks(prev => [...prev, { name: btn.label, price: btn.price }]);
        setDrinkPriceCache(prev => ({ ...prev, [btn.label]: btn.price })); // Cache the price for display
        setDynamicButtons([]); // Clear the dynamic buttons
        setCurrentCategory(null); // Reset the category
  ...
  function editCombo(idx) {
    // Mark the combo as incomplete to allow editing
    setCombos(prev => {
      const copy = prev.map((c, i) => ({
        ...c,
        completed: i === idx ? false : c.completed
      }));
      return copy;
    });
    
    // Enable the bottom menu and show sides by default
    setBottomMenuEnabled(true);
    setCurrentCategory('sides');
    getSides();
  }
      ...
      <header className="top-bar">
        ...
        <div className="header-right">
          ...
          <button
            className="manager-portal-button"
            onClick={() => {
              const url = window.location.origin + '/inventory-board';
              window.open(url, '_blank');
            }}
          >
            Inventory Board
        ...
        <div className="main-area" style={{flex: 1, minWidth: 0}}>
          <div className="top-controls">
            ...
            <div className="action-row">
              <button onClick={onAddDrink} disabled={hasIncompleteCombo}>Add Drink</button>
              <button onClick={onAppetizersSelected} disabled={hasIncompleteCombo}>Appetizers</button>
