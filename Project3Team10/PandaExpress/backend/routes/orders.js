...
// Helper function to generate a parseable order string
function generateOrderString(combos, appetizers, drinks) {
  const parts = [];

  // Format combos
  for (const combo of combos) {
    const entrees = combo.items.filter(item => !combo.sides || !combo.sides.includes(item));
    const sides = combo.sides || [];
    
    let comboStr = `${combo.size}:`;
    if (entrees.length > 0) {
      comboStr += `E(${entrees.join(',')})`;
    }
    if (sides.length > 0) {
      comboStr += `S(${sides.join(',')})`;
    }
    parts.push(comboStr);
  }
  ...
  if (appetizers && appetizers.length > 0) {
    const appNames = appetizers.map(app => app.name || app).join(',');
    parts.push(`APP(${appNames})`);
  }

  // Format drinks
  if (drinks && drinks.length > 0) {
    const drinkNames = drinks.map(drink => drink.name || drink).join(',');
    parts.push(`DRK(${drinkNames})`);
  }
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const orderString = generateOrderString(combos, appetizers, drinks);

    // Insert order with order_string
    const orderResult = await pool.query(
      "INSERT INTO orders (timestamp, cost, order_string) VALUES (NOW(), $1, $2) RETURNING id",
      [totalPrice, orderString]
    );
