...
export default function Customer({ onReturnToCashier, onNavigateToManager }){
    const options = [
        'Bowl',
        'Plate',
        'Bigger Plate',
    ...
    // Open the previous modal-style size selector (kept for backward compatibility)
    function openSizeModal(sizeName){
        // kept but no longer used by UI — sizes now use full-page flow
        setModalMode('size');
    ...
    async function openAppetizerPage(){
        ...
        try {
            ...
            const resp = await fetch('/api/appetizers');
            if (resp.ok){
    ...
    // Load appetizer images as blobs and create object URLs (id -> object URL)
    async function loadAppetizerImages(items = []){
        // No-op: prefer direct <img src="/api/appetizer-drink-image/:id"> rendering to avoid
        // repeated reflows and object URL churn which caused UI jitter.
        return;
    }

    async function openAppetizerModal(){
        setModalMode('appetizer');
        setModalSize(null);
        setSelectedSides([]);
        setSelectedEntrees([]);
        setSelectedAppetizer(null);
        ...
        try {
            ...
            console.error('openAppetizerModal error', e);
            setAvailableAppetizers([]);
        }
    }
    ...
    const total = orderItems.reduce((s, it) => s + (Number(it.price) || 0), 0);
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        ...
        const initGoogleTranslate = () => {
          if (translateLoaded) return; // prevent double initializations
    ...
    function checkout(){
        ...
        (async () => {
            ...
            const combos = orderItems
                ...
                .map(it => ({ size: it.name, items: it.entrees && it.entrees.length > 0 ? it.entrees : [] }));
            ...
            <header className="site-header" role="banner">
                ...
                <div className="header-actions">
                    <button
                        ...
                        onClick={() => { if (onNavigateToManager) onNavigateToManager(); else window.alert('Manager view not available'); }}
                    ...
                    <button
                        type="button"
                        className="btn btn-secondary"
                        onClick={() => { if (onReturnToCashier) onReturnToCashier(); else window.alert('Cashier view not available'); }}
